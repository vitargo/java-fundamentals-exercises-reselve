...
public class CrazyStreams {
    ...
    public List<Account> sortByFirstAndLastNames() {
        ...
        return accounts.stream()
                .sorted(compareByFirstName.thenComparing(compareByLastName))
                .collect(Collectors.toList());
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        Optional<Account> result = accounts.stream()
                ...
                .findFirst();
        return result.orElseThrow(() -> new EntityNotFoundException(String.format("Cannot find Account by email={%s}", email))).getBalance();
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(groupingBy(account -> account.getCreationDate().getMonth(), mapping(Account::getBalance, reducing(new BigDecimal(0), BigDecimal::add))));
    }
