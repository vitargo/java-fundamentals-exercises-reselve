...
public class CrazyOptionals {
    ...
    public static Optional<Account> retrieveAccountGmail(AccountProvider accountProvider) {
        return accountProvider.getAccount().filter(account -> account.getEmail().substring(account.getEmail().indexOf('@') + 1).equals("gmail.com"));
    }
    ...
    public static Account getAccountWithMaxBalance(List<Account> accounts) {
        ...
        return accounts.stream().max(comparator).orElseThrow(NoSuchElementException::new);
    }
    ...
    public static OptionalDouble findMinBalanceValue(List<Account> accounts) {
        return accounts.stream().min(Comparator.comparing(Account::getBalance)).map(account -> OptionalDouble.of(account.getBalance().doubleValue())).orElse(OptionalDouble.empty());
    }
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream()
                .flatMapToDouble(account -> DoubleStream.of(account.getCreditBalance().map(BigDecimal::doubleValue).orElse(0D)))
